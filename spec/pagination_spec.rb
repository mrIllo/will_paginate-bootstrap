...
describe 'Bootstrap Renderer' do
  # Render pagination for the middle page so that we can test for the presence @
  ...
  it 'has an anchor within each non-active/non-disabled list item' do
    html.css('ul li:not(.active):not(.disabled)').each do |li|
      li.at_css('a').wont_be_nil
    end
  end
  ...
  it 'uses an anchor to wrap the ellipsis' do
    ellipsis = BootstrapPagination::BootstrapRenderer::ELLIPSIS
    html.at_css('li.disabled a', text: ellipsis).wont_be_nil
  end
